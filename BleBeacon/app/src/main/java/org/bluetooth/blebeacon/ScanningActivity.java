...
//            final Intent intent = new Intent(this, PeripheralActivity.class);
            final Intent intent = new Intent(this, LinkedinView.class);
    ...
    private void handleFoundDevice(final BluetoothDevice device, final int rssi, final byte[] scanRecord, final String devid)
	{
		...
		runOnUiThread(new Runnable() 
		{
			...
			public void run() 
			{
				...
				if (mBeacon)
				{
					mDevicesListAdapter.addDevice(device, rssi, scanRecord, devid); //TODO: Add to database or make into webview for linkedin integration.
					mDevicesListAdapter.notifyDataSetChanged();
				}
			}
		});
